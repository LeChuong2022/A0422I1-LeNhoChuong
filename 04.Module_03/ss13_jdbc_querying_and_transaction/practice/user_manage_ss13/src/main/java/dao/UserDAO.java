...
public class UserDAO implements IUserDAO {
    ...
    public void addUserTransaction(User user, int[] permissions) {
        ...
        try {
            ...
            if (rs.next())
                userID = rs.getInt(1);
            // in case the insert operation success, assign permision to user
            if (rowAffected == 1) {
                // assign permission to user
                String sqlPivot = "insert into user_permission(user_id, permisson_id) values(?,?);";
    ...
    public void insertUpdateWithoutTransaction() {
        try (Connection conn = getConnection();
             ...
             PreparedStatement psUpdate = conn.prepareCall(SQL_UPDATE)) {
